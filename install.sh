#!/bin/sh
# install.sh — install lockstep from GitHub.
#
#   curl -fsSL https://raw.githubusercontent.com/srivtx/lockstep/main/install.sh | sh
#
# Installs the `lockstep` binary globally with Bun. Pass a git ref as the
# first argument to pin a branch, tag, or commit:
#
#   curl -fsSL https://raw.githubusercontent.com/srivtx/lockstep/main/install.sh | sh -s v0.2.0
set -eu

REPO="srivtx/lockstep"
BIN="lockstep"
REF="${1:-main}"
